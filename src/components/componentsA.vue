<template>
  <div class="componentsA">
      <P class="title">组件A</P>
      <P class="titleName">餐馆名称：{{resturantName}}  AAAA：{{chenquan}}</P>
      <div>
            <!-- 点击修改 为 A 餐馆 -->
      ...
      <div>
            <!-- 点击修改 为 A 餐馆 -->
          <button class="btn" @click="changeChenquan('A餐馆','B餐厅')">修改陈铨测试值</button>
      </div>
      <div class="marTop">
...
export default {
  ...
  methods:{
      ...mapActions( // 语法糖
          ['modifyAName','changeChenquan'] // 相当于this.$store.dispatch('modifyName'),提交这个方法
      ),
      trunToB () {
